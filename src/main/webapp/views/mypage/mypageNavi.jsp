      ...
      <p class="mypage-sbn-menu">
       <c:choose>
            <c:when test="${param.page == 'userInfo'}">
              <a href="<%=request.getContextPath()%>/jelly?page=userInfo" style="color:black; font-weight: bold">로그인 관리</a>
            </c:when>
            <c:otherwise>
              <a href="<%=request.getContextPath()%>/jelly?page=userInfo">로그인 관리</a>
            </c:otherwise>
      </c:choose>
      </p>
